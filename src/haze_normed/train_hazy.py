...
class Dataset(torch.utils.data.Dataset):
	...
	def __getitem__(self, index):
		...
		cv_out = cv2.imread('../img/'+self.filenames[ID]+'_trans.jpg',0) # grayscale
		#if cv_in is None:
...
def main():
	...
	train_dataset = Dataset(ids_train, files_train, transforms.Compose([transforms.Resize((256,256)),transforms.ToTensor(),transforms.Normalize(mean=[0.5231, 0.5180, 0.5115],std=[0.2014, 0.2018, 0.2100]),]), transforms.Compose([transforms.Resize((256,256)),transforms.ToTensor(),transforms.Normalize(mean=[0.5238],std=[0.1931]),])) # normalize
	...
	for epoch in range(20):
		...
		if loss < best_loss:
			torch.save(model.state_dict(),r'model_hazy_best.pth')
			best_loss = loss
